//
//  listdev.cpp
//  A sample program to list PL27A1 device and show its information
//
...
int main(void) {
  libusb_device**
      devices;  // Structures representing all USB device detected on the system
  ...
  libusb_device*
      device;  // Structure representing one USB device detected on the system
  int i = 0, j = 0, k = 0;
  uint8_t path[8];  // The array that should contain the port numbers
  struct libusb_device_descriptor
      device_descriptor;  // structure represents the standard USB device
                          // descriptor
  struct libusb_config_descriptor* config_desc =
      NULL;  // Structure represents the standard USB configuration descriptor
  struct libusb_interface_descriptor
      interface_descriptor;  // A structure represents the standard USB
                             // interface descriptor
  struct libusb_endpoint_descriptor
      endpoint;  // A structure represents the standard USB endpoint descriptor
  struct libusb_device_handle* dev_handle =
      NULL;  // Structure represents the handles on a USB device
  ...
  while ((device = devices[i++]) != NULL) {
    ...
    printf("%04x:%04x (bus %d, device %d)", device_descriptor.idVendor,
           device_descriptor.idProduct, libusb_get_bus_number(device),
    ...
    if ((OURSTECH_VID == device_descriptor.idVendor) &&
        (IZEC35_7301_PID == device_descriptor.idProduct)) {
      // open the target device
      DEBUG("(%s, %s(), L%d)\n", __FILE__, __FUNCTION__, __LINE__);
      printf(
          "--------------------------------------------------------------\n");
      printf("    PL27A1 USB device information\n");
      ...
      DEBUG("(%s, %s(), L%d) config_desc->bNumInterfaces = %d\n", __FILE__,
            __FUNCTION__, __LINE__, config_desc->bNumInterfaces);

      // Find all interfaces in PL27A1
      for (j = 0; j < config_desc->bNumInterfaces; j++) {
        memcpy(&interface_descriptor,
               &(config_desc->interface[j].altsetting[0]),
               sizeof(struct libusb_interface_descriptor));
        DEBUG("(%s, %s(), L%d)\n", __FILE__, __FUNCTION__, __LINE__);
        printf("    No. of endpoints = %d\n",
               interface_descriptor.bNumEndpoints);

        // Find all endpoints in PL27A1
        for (k = 0; k < interface_descriptor.bNumEndpoints; k++) {
          memcpy(&endpoint, &interface_descriptor.endpoint[k],
                 sizeof(struct libusb_endpoint_descriptor));
          DEBUG("(%s, %s(), L%d) endpoint.bmAttributes = %x\n", __FILE__,
                __FUNCTION__, __LINE__, endpoint.bmAttributes);
